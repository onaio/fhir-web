...
var HeaderBreadCrumb = function HeaderBreadCrumb(props) {
  ...
  var handleTabLink = function handleTabLink(key) {
    if (key === 'credentials') {
      _connectedReducerRegistry.history.push("".concat(_constants.URL_USER_CREDENTIALS, "/").concat(userId));
    } else if (key === 'details') {
      _connectedReducerRegistry.history.push("".concat(_constants.URL_USER_EDIT, "/").concat(userId));
    } else {
  ...
  return !userId ? React.createElement(_antd.Tabs, {
    type: "card",
