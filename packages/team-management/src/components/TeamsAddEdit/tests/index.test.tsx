...
import { accessToken, id, intialValue, practitioners, team, team1 } from './fixtures';
...
describe('Team-management/TeamsAddEdit/TeamsAddEdit', () => {
  ...
  it('render correctly when team name in is not provided', async () => {
    fetch.mockResponseOnce(JSON.stringify(team1));
    fetch.mockResponseOnce(JSON.stringify(practitioners));
    fetch.mockResponseOnce(JSON.stringify(practitioners));
    ...
    expect(wrapper.find('.mb-3.header-title').text()).toEqual(`Edit Team`);
  });
});
